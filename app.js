...
(() => {
  ...
  function reRenderPairTable(pairKey) {
    ...
    const sources = getSourcesForFilter(filter, reports);
  ...
  function renderDiffTable(pair) {
    ...
    if (!diffState.has(key)) {
      diffState.set(key, { filter: "all", page: 1, pageSize: 50 });
    }
    ...
    return `${tabsHtml}${truncatedNote}${paginationHtml}<div class="diff-table-wrap"><table class="diff-table">${thead}<tbody></tbody></table></div>`;
  }
