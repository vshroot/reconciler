...
(() => {
  ...
  // Per-pair row cache and pagination/filter state (keyed by pairKey string).
  const diffRowsCache = new Map(); // pairKey → rows[]
  ...
  function buildDiffRows(reports, keepCols, amountScale) {
    const rows = [];
  ...
  function reRenderPairTable(pairKey) {
    ...
    if (!pairBlock) return;
    const allRows = diffRowsCache.get(pairKey) || [];
    const st      = diffState.get(pairKey);
    if (!st) return;
    const { filter, page, pageSize } = st;

    // 1. Filter.
    const filtered = filter === "all" ? allRows : allRows.filter((r) => r._rowType === filter);
    ...
    const pageRows = filtered.slice(start, useAll ? total : start + size);

    // 3. Re-render tbody.
    const keepCols = pairBlock._keepCols || [];
  ...
  function renderDiffTable(pair) {
    ...
    const rows = buildDiffRows(reports, keepCols, pair.amountScale);

    // Cache rows for pagination re-renders.
    diffRowsCache.set(pair.key, rows);
    if (!diffState.has(pair.key)) {
      diffState.set(pair.key, { filter: "all", page: 1, pageSize: 50 });
    }
    ...
    // Initial render: first 50 rows of "all".
    const initRows   = rows.slice(0, 50);
    const totalPages = Math.ceil(rows.length / 50);
    const multiPage  = rows.length > 50;
    ...
    const paginationHtml = `
      <div class="diff-pagination">
        ...
        <div class="diff-page-nav">
          ...
          <span class="diff-page-indicator muted">${multiPage ? `Page 1 of ${totalPages}` : `${rows.length} row${rows.length !== 1 ? "s" : ""}`}</span>
          <button class="diff-nav-btn" type="button" data-page-next="1" aria-label="Next" ${multiPage ? "" : "disabled"}>&#8594;</button>
        </div>
      </div>`;
    ...
    const tbodyHtml = initRows.map((r) => renderDiffRow(r, keepCols)).join("");

    return `${tabsHtml}${truncatedNote}${paginationHtml}<div class="diff-table-wrap"><table class="diff-table">${thead}<tbody>${tbodyHtml}</tbody></table></div>`;
  }
  ...
  function renderResults(summary, downloads) {
    ...
    // Attach keepCols as a DOM expando so reRenderPairTable can access it without a closure.
    for (const pair of summary.pairs) {
      const el = resultsEl.querySelector(`[data-pair-key="${CSS.escape(pair.key)}"]`);
      if (el) el._keepCols = pair.keepCols || [];
    }
  }
  ...
  async function run() {
    ...
    // Reset diff table caches for the new run.
    diffRowsCache.clear();
    diffState.clear();
  ...
  resultsEl.addEventListener("click", (ev) => {
    ...
    if (nextBtn && !nextBtn.disabled) {
      ...
      if (!st) return;
      const allRows   = diffRowsCache.get(pairKey) || [];
      const filtered  = st.filter === "all" ? allRows : allRows.filter((r) => r._rowType === st.filter);
      const totalPages = st.pageSize === "all" ? 1 : Math.ceil(filtered.length / st.pageSize);
      if (st.page < totalPages) { st.page += 1; reRenderPairTable(pairKey); }
      return;
    }
  });
