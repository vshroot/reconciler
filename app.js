...
(() => {
  ...
  function validate() {
    ...
    for (const f of state.files) {
      if (!f.file) errs.push(`File not selected: ${f.name}`);
      if (!f.delimiter || String(f.delimiter).length !== 1) errs.push(`Invalid delimiter for ${f.name}`);
      if (!f.header || !f.header.length) errs.push(`Could not read header for ${f.name}`);
      if (f.idCol < 0 || f.amountCol < 0 || f.statusCol < 0) errs.push(`Select id/amount/status columns for ${f.name}`);
    }
